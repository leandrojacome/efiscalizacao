...
@section('titulo', "Editar {$usuario->nome}")
    ...
    <div class="col-lg-12">
        ...
        <div class="card">
            ...
            <div class="card-body card-block">
                <form name="form1" action="{{ route('usuario.update', $usuario->id) }}" method="POST" class="form-horizontal">
                    @csrf
                    {{method_field('patch')}}
                    @include('usuario.form')
                </form>
                <form name="form2" action="{{ route('usuario.destroy', $usuario->id) }}" method="POST" class="form-horizontal">
                    @csrf
                    {{method_field('delete')}}
                </form>
            </div>
